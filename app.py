...
def apply_custom_ui():
    ...
    <style>
        ...
        /* Centered Cell for Price/Target */
        .compact-cell { 
            font-size: 0.8rem; 
            white-space: nowrap; 
            overflow: hidden; 
    ...
    with tab_alerts:
        ...
        with col_list:
            ...
            if not active_view.empty:
                # --- TABLE VIEW (IMPROVED ALIGNMENT) ---
                if view_mode == "📄 Table":
                    # Headers: [Ticker(1.3), Target(1.1), Current(1.1), Actions(0.9)]
                    # Adjusted to give max space to data, minimizing buttons
                    h1, h2, h3, h4 = st.columns([1.3, 1.1, 1.1, 0.9]) 
                    h1.markdown("<div class='tbl-header' style='text-align:left; padding-left:5px;'>SYM</div>", unsafe_allow_html=True)
                    ...
                    for idx, row in active_view.iterrows():
                        ...
                        # Left Align Ticker, Center Align Prices
                        ...
                        st.markdown("<hr style='margin: 2px 0; border-color: #333; opacity: 0.5;'>", unsafe_allow_html=True)

                # --- CARD VIEW ---
                else:
                    for idx, row in active_view.iterrows():
                        st.markdown(f"""<div style="background:#F9E79F; padding:8px; border-radius:6px; margin-bottom:5px; color:black;"><div style="display:flex; justify-content:space-between;"><b>{row['ticker']}</b> <b>${float(row['target_price']):.2f}</b></div><small>{row['direction']} | {row['notes']}</small></div>""", unsafe_allow_html=True)
